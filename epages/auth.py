...
def calculate_signature(code, access_token_url, client_secret):
    ...
    digest = hmac.new(client_secret, msg=message.encode('utf-8'), digestmod=hashlib.sha256).digest()
    return base64.b64encode(digest).decode()

def verify_signature(code, access_token_url, client_secret, signature):
    '''Handy
    '''
    return signature == calculate_signature(code.encode('utf-8'), access_token_url.encode('utf-8'), client_secret.encode('utf-8'))
